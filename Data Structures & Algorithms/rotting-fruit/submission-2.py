class Solution:
    ...
    def orangesRotting(self, grid: List[List[int]]) -> int:
        ...
        while (num_fresh > 0):
            ...
            num_minutes+=1
        return num_minutes
        #post question thoughts
        # a queue or deque wouldve been better here, because I wouldn't 
        # need to make an auxillary copy everytime. 
        # instead, my "copy" with the deque idea is just specifying how many
        # elements i want to prcoess at a time with the current size of the deque 
        # also i never knew but this check is O(1) 
        # if (row in range(len(grid))and col in range(len(grid[0])))
        # much better for in bounds checking
